...
import { getServiceMeta as getIconServiceMeta } from './service-icons.js';

/**
 * Get service icon/color from issuer name
 * @param {string} issuer
 * @returns {{bg: string, letter: string, emoji: string|null, hasLogo: boolean, svg: string}}
 */
export function getServiceMeta(issuer) {
  return getIconServiceMeta(issuer);
}
